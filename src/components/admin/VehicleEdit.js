...
const VehicleEdit = () => {
  ...
  const onSubmit = async (values) => {
    ...
    var imageId = ""; // imageId kullanabilmek icin en yakin scopte tanimlamak gerekli!!!!!
    ...
    try {
      if (isNewImage) {
        ...
        const respUpload = await uploadVehicleImage(formData); // = uploadVehicleImage(formData).then(resp{ }) İkisi ayni islemi yapiyor ama ard arta birden fazla data gelcekse ilk kullanim daha dogru olur
      ...
      if (respVehicle.status !== 200) throw respVehicle.response.data.message;
      setLoading(false);
      toast("The vehicle was updated succesfully");
    } catch (err) {
  ...
  const handleImageChange = () => {
    ...
    if (!file) return; //File null ise

    formik.setFieldValue("image", file); //Eklenen dosyaları file klasorunden alıp imageye ekleyecek
  ...
  const handleSelectImage = (params) => {
    fileImageRef.current.click(); //Objeye tiklanmasini sagliyoruz
  };
    ...
    alertify.confirm(
      ...
      () => {
        ...
        deleteVehicle(vehicleId)
          .then((resp) => {
            toast("Vehicle was deleted succesfully");
            setDeleting(false);
